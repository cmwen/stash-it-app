...
class AppDatabase extends _$AppDatabase {
  ...
  Future<ArticlesTableData?> getArticleById(String id) {
    return (select(articlesTable)..where((t) => t.id.equals(id)))
        .getSingleOrNull();
  }

  Future<ArticlesTableData?> getArticleByUrl(String url) {
    return (select(articlesTable)..where((t) => t.url.equals(url)))
        .getSingleOrNull();
  }
  ...
  Future<bool> updateArticle(ArticlesTableCompanion article) {
    return update(articlesTable).replace(ArticlesTableData(
      id: article.id.value,
  ...
  Future<void> markAsRead(String id) async {
    await (update(articlesTable)..where((t) => t.id.equals(id)))
        .write(const ArticlesTableCompanion(isRead: Value(true)));
  }

  Future<void> markAsUnread(String id) async {
    await (update(articlesTable)..where((t) => t.id.equals(id)))
        .write(const ArticlesTableCompanion(isRead: Value(false)));
  }
  ...
  Future<void> toggleFavorite(String id) async {
    ...
    if (article != null) {
      await (update(articlesTable)..where((t) => t.id.equals(id)))
          .write(ArticlesTableCompanion(isFavorite: Value(!article.isFavorite)));
    }
  }
  ...
  Future<void> toggleArchive(String id) async {
    ...
    if (article != null) {
      await (update(articlesTable)..where((t) => t.id.equals(id)))
          .write(ArticlesTableCompanion(isArchived: Value(!article.isArchived)));
    }
  }

  Future<void> updateScrollPosition(String id, double position) async {
    await (update(articlesTable)..where((t) => t.id.equals(id)))
        .write(ArticlesTableCompanion(scrollPosition: Value(position)));
  }

  Future<void> updateStatus(String id, String status) async {
    await (update(articlesTable)..where((t) => t.id.equals(id)))
        .write(ArticlesTableCompanion(status: Value(status)));
  }
